...
class xAuth {
    ...
    public function ajaxLogin($url = '/', $email, $passwd){
        ...
        if (!$email || !$passwd){
            ...
            die('{"status":"error","message":"'._("Not all fields are filled").'"}');
        }
        ...
        if(!$user){
            die('{"status":"error","message":"'._("Invalid login or password").'"}');
        }
        if($user && $user->status <= 0){
            die('{"status":"error","message":"'._("Your account is blocked, use the recovery password, or contact the Technical support.").'"}');
        }
        ...
        if ($hash != $cryptpass) {
            ...
            $this->m->_db->query();
            if ($user->bad_auth >= 4) {
                die('{"status":"error","message":"'._("You entered incorrect data 5 times in a row and were blocked. Complete restoration procedure.").'"}');
            }
        ...
        if(!$this->_session->session_id ){
            die('{"status":"error","message":"'._("Session Error").'"}');
        }
