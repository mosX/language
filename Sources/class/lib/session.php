...
class xSession extends DBTable {
    ...
    */
    ...
    function purge( $inc=1800, $and='' ) {
        ...
        //if ($inc == 'core') {
            $past_logged_remember = time() - 31536000; // 365 days = 365*24*60*60
            $past_logged = time() - 2592000; // 30 days = 30*24*60*60
            $past_guest = time() - 14400; // 4 hours = 4*60*60

            $query = "DELETE FROM $this->_tbl"
            . "\n WHERE "
            // purging expired logged sessions
            . "\n ( time < '" . (int)$past_logged_remember . "' "
            . "\n AND remember = 1 "
            . "\n AND (gid = 1 OR gid = 2) "
            . "\n ) OR "
