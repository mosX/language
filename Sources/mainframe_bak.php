...
class mainframe {
    ...
    function run() {
        ...
        $this->_accounts = $this->_auth->getUsersAccount();
        ...
        if($this->isLogin()){
            //$this->playforfun = $_SESSION[$this->_user->id . ".playforfun"];
            
            if($_SESSION[$this->_user->id . ".account"] === null){
                //ищим первый демо акаунт
                foreach($this->_accounts as $item) {
                    if($item->mode == 1) {
                        $this->_user->account = $item->id;
                        break;
                    }
                }
                $_SESSION[$this->m->_user->id . ".account"] = (int)$this->_user->account;
            }else{
                $this->_user->account = $_SESSION[$this->_user->id . ".account"];
            }
            
            foreach($this->_accounts[$this->_user->account] as $key=>$item){    //добавляем значение в массив пользователя
                if($key == 'id') continue;
                $this->_user->{$key} = $item;
            }
            
            //GET OFFERS ACTIVE TYPES
            if($this->_user->mode == 4){
                $this->_db->setQuery(
                            "SELECT `offers`.`types` "
                            . " FROM `offers` "
                            . " WHERE `offers`.`id` = ".$this->_user->offer_id
                            . " LIMIT 1"
                        );
                $types = $this->_db->loadResult();
                $arr = explode(',',$types);
                foreach ($arr as $item){
                    $this->_user->offer_types[$item] = $item;
                }
            }

            //проверяем минимальную и максимальну. ставку взависимости от режима
            $this->getDefaultBet();
        $this->_offer = $this->getOffers();
        ...
        if ($this->protocol == false) {   //если вебсокеты отключены
            $this->info = $this->get4Info(); //последние значение катировок для информации в хедере 
        }
